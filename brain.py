...
<iframe title="Litigation Propensity Model Monitoring Dashboard" width="1200" height="800" src="https://app.powerbi.com/reportEmbed?reportId=05566686-41f7-46ff-a15d-df70a00f4f3f&autoAuth=true&ctid=dafe49bc-5ac3-4310-97b4-3e44a28cbf18" frameborder="0" allowFullScreen="true"></iframe>  
    ...
    if strategy == "Outcome Focused":
        ...
        if Lit_data_firm_filter_df.empty:
            st.warning("No firms meet the selected allocation threshold.")
        else:
            Lit_data_firm_filter_df = Lit_data_firm_filter_df.copy()



        # ---- Compute weighted score ----
        ct = Lit_data_firm_filter_df['Cycle time']
        ...
        #     cycle_time_weight * (np.log(Lit_data_firm_filter_df['Cycle time']+1)) +
        #     win_rate_weight * (Lit_data_firm_filter_df['Win rate proxy']) +
        #     Closed_case_cnt_weight * (np.log(Lit_data_firm_filter_df['Case closed count']+1))
        # )
    ...
    # e.g., 0.84 for 84%
        ...
        #     paid_post_appeal * (np.log(Lit_data_firm_filter_df['Paid post appeal']+1))
        #     )
